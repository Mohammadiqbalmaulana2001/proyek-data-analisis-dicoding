...
class BrazilMapPlotter:
    ...
    def plot(self):
        try:
            ...
            with self.urllib.request.urlopen('https://i.pinimg.com/originals/3a/0c/e1/3a0ce18b3c842748c255bc0aa445ad41.jpg') as url:
                brazil = self.mpimg.imread(url, 'jpg')
        except Exception as e:
            self.st.error(f"Error fetching image: {e}")
            return

        fig, ax = self.plt.subplots(figsize=(10, 10))
        self.data.plot(kind="scatter", x="geolocation_lng", y="geolocation_lat", 
                        alpha=0.8, s=0.3, color='#ff0000', ax=ax)
        ax.set_title('Scatter Plot of Customers in Brazil', fontsize=30)
